...
- **업데이트 확인은 채널을 따른다.** 정식 설치본은 GitHub `/releases/latest`만 보고, 사전 출시 설치본(버전에 PEP 440 pre 구간)은 릴리스 목록(`/releases?per_page=30`)에서 초안과 `updater-3x-pre` 같은 버전 없는 태그를 건너뛰고 가장 높은 버전을 고른다(`crates/impulcifer-service/src/update/check.rs`). `/releases/latest`는 사전 출시를 절대 돌려주지 않으므로 이 규칙을 되돌리면 alpha 설치본이 다음 alpha를 못 본다.
- **Python 휠에는 오디오 백엔드가 없다.** Python 표면에 녹음 진입점이 없으므로 `crates/impulcifer-python`은 `impulcifer-service`·`impulcifer-cli`를 `default-features = false`로 쓰고(`native-audio` 제외, `NullBackend`), Linux 휠은 `PyO3/maturin-action`의 manylinux_2_28 컨테이너에서 `--auditwheel check`로 빌드해 PyPI에 올린다. 휠 경로에 cpal·`impulcifer-sys-win`이 다시 들어오면 manylinux 검사가 실패한다.
- **휠 RECORD와 sdist는 스크립트가 맞춘다.** maturin은 RECORD 경로를 CSV로 인용하지 않아 `sweep-seg-FL,FR-...wav` 줄이 깨지고 PyPI가 그런 휠을 거부하므로, `maturin build` 뒤에는 `crates/impulcifer-python/repair_record.py`를 돌린다(워크플로·`verify.py`). pip 빌드는 `build_backend.py`(maturin 훅 + 보정)를 거친다. `impulcifer-service`가 `include_str!`로 읽는 `i18n/locales`는 크레이트 밖이라 `sync_data.py`가 크레이트에 스테이징해야 sdist 최상위에 들어간다. `rust.yml`의 `sdist` 잡이 pip로 sdist를 빌드해 휠 테스트를 돌린다.

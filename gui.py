def main_gui():
        ...
        try:
            ...
            if font_path and os.path.exists(font_path):
                try:
                    ...
                    print(
                        f"GUI용 Pretendard 폰트 등록됨: {actual_font_name} ({font_path})"
                    )
    ...
    try:
        tkinter.font.nametofont("TkDefaultFont").configure(
            family=default_font[0], size=default_font[1]
        )
        tkinter.font.nametofont("TkTextFont").configure(
            family=default_font[0], size=default_font[1]
        )
        tkinter.font.nametofont("TkFixedFont").configure(
            family=default_font[0], size=default_font[1]
        )
        tkinter.font.nametofont("TkMenuFont").configure(
            family=default_font[0], size=default_font[1]
        )
    ...
    def refresh1(init=False):
        ...
        for host in host_apis.values():
            host_api_optionmenu["menu"].add_command(
                label=host, command=tkinter._setit(host_api, host)
            )
        ...
        for device in output_devices:
            output_device_optionmenu["menu"].add_command(
                label=device, command=tkinter._setit(output_device, device)
            )
        for device in input_devices:
            input_device_optionmenu["menu"].add_command(
                label=device, command=tkinter._setit(input_device, device)
            )
        ...
        if channels_check.get():
            ...
            try:
                ...
                print(
                    f"Channels entry enabled - Recording will use {channel_count} channels"
                )
            except Exception:
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        Label(canvas1, text="Playback device"), pos, maxwidth, maxheight
    )
    output_device_optionmenu = OptionMenu(
        canvas1, variable=output_device, value=None, command=refresh1
    )
    widgetpos, pos, maxwidth, maxheight = pack(
        output_device_optionmenu, pos, maxwidth, maxheight, samerow=True
    )
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        Label(canvas1, text="Recording device"), pos, maxwidth, maxheight
    )
    input_device_optionmenu = OptionMenu(
        canvas1, variable=input_device, value=None, command=refresh1
    )
    widgetpos, pos, maxwidth, maxheight = pack(
        input_device_optionmenu, pos, maxwidth, maxheight, samerow=True
    )

    # host API
    widgetpos, pos, maxwidth, maxheight = pack(
        Label(canvas1, text="Host API"), pos, maxwidth, maxheight
    )
    host_api = StringVar()
    host_api.trace("w", lambda *args: refresh1())
    host_api_optionmenu = OptionMenu(canvas1, host_api, value=None, command=refresh1)
    widgetpos, pos, maxwidth, maxheight = pack(
        host_api_optionmenu, pos, maxwidth, maxheight, samerow=True
    )

    # sound file to play
    widgetpos, pos, maxwidth, maxheight = pack(
        Label(canvas1, text="File to play"), pos, maxwidth, maxheight
    )
    play = StringVar(
        value=os.path.join(
    ...
    # output file
    widgetpos, pos, maxwidth, maxheight = pack(
        Label(canvas1, text="Record to file"), pos, maxwidth, maxheight
    )
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        Button(canvas1, text="...", command=lambda: savefile(record)),
        pos,
        maxwidth,
        maxheight,
        samerow=True,
    ...
    channels_checkbutton = Checkbutton(
        canvas1, text="Channels", variable=channels_check
    )
    widgetpos, pos, maxwidth, maxheight = pack(
        channels_checkbutton, pos, maxwidth, maxheight
    )
    ToolTip(
        channels_checkbutton,
        "For room correction: some measurement microphones like MiniDSP UMIK-1 are seen as stereo microphones by Windows and will for that reason record a stereo file. recorder can force the capture to be one channel. For multi-channel HRIR recording: Check this and set the number of input channels (2 channels per speaker for left/right ears). Common configurations: 14 channels (7 speakers), 22 channels (11 speakers, 7.0.4), 26 channels (13 speakers, 7.0.6).",
    )
    channels = IntVar(value=14)
    channels.trace("w", lambda *args: update_channel_guidance())
    channels_entry = Entry(
        canvas1,
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        channels_entry, pos, maxwidth, maxheight, samerow=True
    )
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        channel_guidance_label, pos, maxwidth, maxheight
    )
    ...
    ToolTip(
        append_check,
        "Add track(s) to existing file. Silence will be added to end of each track to make all equal in length.",
    )
    ...
    widgetpos, pos, maxwidth, maxheight = pack(
        Button(canvas1, text="RECORD", command=recordaction), pos, maxwidth, maxheight
    )
    ...
    def refresh2(changedpath=False):
        if changedpath:
            if os.path.exists(dir_path.get()):
                ...
                s = ";".join(files)
                if re.search(
                    r"\broom(-[A-Z]{2}(,[A-Z]{2})*-(left|right))?\.wav\b", s, re.I
                ):
        ...
        if do_room_correction.get():
            do_room_correction_msg_label.place(
                x=label_pos[do_room_correction_msg_label][0],
                y=label_pos[do_room_correction_msg_label][1],
                anchor=W,
            )
        else:
            do_room_correction_msg_label.place_forget()
        if do_headphone_compensation.get():
            do_headphone_compensation_msg_label.place(
                x=label_pos[do_headphone_compensation_msg_label][0],
                y=label_pos[do_headphone_compensation_msg_label][1],
                anchor=W,
            )
        else:
            do_headphone_compensation_msg_label.place_forget()
        if do_equalization.get():
            do_equalization_msg_label.place(
                x=label_pos[do_equalization_msg_label][0],
                y=label_pos[do_equalization_msg_label][1],
                anchor=W,
            )
        else:
            do_equalization_msg_label.place_forget()

        specific_limit_entry.config(
            state=NORMAL if do_room_correction.get() else DISABLED
        )
        generic_limit_entry.config(
            state=NORMAL if do_room_correction.get() else DISABLED
        )
        room_target_entry.config(state=NORMAL if do_room_correction.get() else DISABLED)
        room_mic_calibration_entry.config(
            state=NORMAL if do_room_correction.get() else DISABLED
        )
        fr_combination_method_optionmenu.config(
            state=NORMAL if do_room_correction.get() else DISABLED
        )
        headphone_compensation_file_entry.config(
            state=NORMAL if do_headphone_compensation.get() else DISABLED
        )
        ...
        mic_deviation_strength_entry.config(
            state=NORMAL if microphone_deviation_correction.get() else DISABLED
        )

        if show_adv.get():
            for widget in adv_options_pos:
                widget.place(
                    x=adv_options_pos[widget][0], y=adv_options_pos[widget][1], anchor=W
                )

            if channel_balance.get() == "number":
                channel_balance_db_entry.place(
                    x=adv_options_pos[channel_balance_db_entry][0],
                    y=adv_options_pos[channel_balance_db_entry][1],
                    anchor=W,
                )
                channel_balance_db_label.place(
                    x=adv_options_pos[channel_balance_db_label][0],
                    y=adv_options_pos[channel_balance_db_label][1],
                    anchor=W,
                )
            else:
            ...
            if decay_per_channel.get():
                for i in range(7):
                    decay_labels[i].place(
                        x=adv_options_pos[decay_labels[i]][0],
                        y=adv_options_pos[decay_labels[i]][1],
                        anchor=W,
                    )
                    decay_entries[i].place(
                        x=adv_options_pos[decay_entries[i]][0],
                        y=adv_options_pos[decay_entries[i]][1],
                        anchor=W,
                    )
            else:
                for i in range(7):
    ...
    # your recordings
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Label(canvas2, text="Your recordings"), pos2, imp_maxwidth, imp_maxheight
    )
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        dir_path_entry, pos2, imp_maxwidth, imp_maxheight
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Button(canvas2, text="...", command=lambda: opendir(dir_path)),
        pos2,
        imp_maxwidth,
        imp_maxheight,
        samerow=True,
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        test_signal_label, pos2, imp_maxwidth, imp_maxheight
    )
    test_signal = StringVar(
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        test_signal_entry, pos2, imp_maxwidth, imp_maxheight
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Button(
            canvas2,
    ...
    do_room_correction_checkbutton = Checkbutton(
        canvas2,
        text="Room correction ",
        variable=do_room_correction,
        command=lambda: refresh2(
            changedpath=True if do_room_correction.get() else False
        ),
    )
    ToolTip(
        do_room_correction_checkbutton,
        "Do room correction from room measurements in format room-<SPEAKERS>-<left|right>.wav located in your folder; e.g. room-FL,FR-left.wav. Generic measurements are named room.wav",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        do_room_correction_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    do_room_correction_msg = StringVar()
    do_room_correction_msg_label = Label(canvas2, textvariable=do_room_correction_msg)
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        do_room_correction_msg_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    ToolTip(
        specific_limit_label,
        "Upper limit for room equalization with speaker-ear specific room measurements. Equalization will drop down to 0 dB at this frequency in the leading octave.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        specific_limit_label, pos2, imp_maxwidth, imp_maxheight
    )
    specific_limit_entry = Entry(
        canvas2,
        textvariable=specific_limit,
        width=5,
        validate="key",
        vcmd=(root.register(validate_int), "%P"),
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        specific_limit_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    generic_limit = IntVar(value=1000)
    genericlimitlabel = Label(canvas2, text="Generic Limit (Hz)")
    ToolTip(
        genericlimitlabel,
        "Upper limit for room equalization with generic room measurements. Equalization will drop down to 0 dB at this frequency in the leading octave.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        genericlimitlabel, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    generic_limit_entry = Entry(
        canvas2,
        textvariable=generic_limit,
        width=5,
        validate="key",
        vcmd=(root.register(validate_int), "%P"),
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        generic_limit_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    fr_combination_method_label = Label(canvas2, text="FR combination method")
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        fr_combination_method_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    fr_combination_methods = ["average", "conservative"]
    fr_combination_method = StringVar(value=fr_combination_methods[0])
    fr_combination_method_optionmenu = OptionMenu(
        canvas2, fr_combination_method, *fr_combination_methods
    )
    ToolTip(
        fr_combination_method_label,
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        fr_combination_method_optionmenu,
        pos2,
        imp_maxwidth,
        imp_maxheight,
        samerow=True,
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        room_mic_calibration_label, pos2, imp_maxwidth, imp_maxheight
    )
    room_mic_calibration = StringVar()
    room_mic_calibration_entry = Entry(
        canvas2, textvariable=room_mic_calibration, width=65
    )
    ToolTip(
        room_mic_calibration_label,
        "Calibration data is subtracted from the room frequency responses. Uses room-mic-calibration.txt (or csv) by default if it exists.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        room_mic_calibration_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Button(
            canvas2,
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        room_target_label, pos2, imp_maxwidth, imp_maxheight
    )
    room_target = StringVar()
    room_target_entry = Entry(canvas2, textvariable=room_target, width=65)
    ToolTip(
        room_target_label,
        "Head related impulse responses will be equalized with the difference between room response measurements and room response target. Uses room-target.txt (or csv) by default if it exists.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        room_target_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Button(
            canvas2,
            text="...",
            command=lambda: openfile(
                room_target, (("Text files", "*.csv *.txt"), ("All files", "*.*"))
            ),
        ),
        pos2,
        imp_maxwidth,
    ...
    do_headphone_compensation_checkbutton = Checkbutton(
        canvas2,
        text="Headphone compensation ",
        variable=do_headphone_compensation,
        command=lambda: refresh2(
            changedpath=True if do_headphone_compensation.get() else False
        ),
    )
    ToolTip(
        do_headphone_compensation_checkbutton,
        "Equalize HRIR tracks with headphone compensation measurement headphones.wav",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        do_headphone_compensation_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    do_headphone_compensation_msg = StringVar()
    do_headphone_compensation_msg_label = Label(
        canvas2, textvariable=do_headphone_compensation_msg
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        do_headphone_compensation_msg_label,
        pos2,
        imp_maxwidth,
        imp_maxheight,
        samerow=True,
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        headphone_compensation_file_label, pos2, imp_maxwidth, imp_maxheight
    )
    headphone_compensation_file = StringVar()
    headphone_compensation_file.trace("w", lambda *args: refresh2(changedpath=True))
    headphone_compensation_file_entry = Entry(
        canvas2, textvariable=headphone_compensation_file, width=65
    )
    ToolTip(
        headphone_compensation_file_label,
        "Headphone compensation measurement file. Uses headphones.wav by default if it exists.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        headphone_compensation_file_entry,
        pos2,
        imp_maxwidth,
        imp_maxheight,
        samerow=True,
    ...
    do_equalization_checkbutton = Checkbutton(
        canvas2,
        text="Custom EQ",
        variable=do_equalization,
        command=lambda: refresh2(changedpath=True if do_equalization.get() else False),
    )
    ToolTip(
        do_equalization_checkbutton,
        "Read equalization FIR filter or CSV settings from file called eq.csv in your folder. The eq file must be an AutoEQ produced result CSV file. Separate equalizations are supported with files eq-left.csv and eq-right.csv.",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        do_equalization_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    do_equalization_msg = StringVar()
    do_equalization_msg_label = Label(canvas2, textvariable=do_equalization_msg)
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        do_equalization_msg_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    label_pos[do_equalization_msg_label] = widgetpos_temp

    # plot
    plot = BooleanVar()
    plot_checkbutton = Checkbutton(
        canvas2, text="Plot results", variable=plot, command=refresh2
    )
    ToolTip(
        plot_checkbutton,
        "Create graphs in your recordings folder (will increase processing time)",
    )
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        plot_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )

    show_adv = BooleanVar()
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Checkbutton(
            canvas2, text="Advanced options", variable=show_adv, command=refresh2
        ),
        pos2,
        imp_maxwidth,
        imp_maxheight,
    ...
    # resample
    fs_check = BooleanVar()
    fs_checkbutton = Checkbutton(
        canvas2, text="Resample to (Hz)", variable=fs_check, command=refresh2
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        fs_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        fs_optionmenu, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        target_level_label, pos2, imp_maxwidth, imp_maxheight
    )
    adv_options_pos[target_level_label] = widgetpos_temp
    target_level = StringVar()
    target_level_entry = Entry(
        canvas2,
        textvariable=target_level,
        width=7,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    ToolTip(
        target_level_label,
        "Normalize the average output BRIR level to the given numeric value. This makes it possible to compare HRIRs with somewhat similar loudness levels. Typically the desired level is several dB negative such as -12.5",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        target_level_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_gain_label, pos2, imp_maxwidth, imp_maxheight
    )
    adv_options_pos[bass_boost_gain_label] = widgetpos_temp
    bass_boost_gain = DoubleVar()
    bass_boost_gain_entry = Entry(
        canvas2,
        textvariable=bass_boost_gain,
        width=7,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_gain_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[bass_boost_gain_entry] = widgetpos_temp

    bass_boost_fc_label = Label(canvas2, text="Fc")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_fc_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[bass_boost_fc_label] = widgetpos_temp
    bass_boost_fc = IntVar(value=105)
    bass_boost_fc_entry = Entry(
        canvas2,
        textvariable=bass_boost_fc,
        width=7,
        validate="key",
        vcmd=(root.register(validate_int), "%P"),
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_fc_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_q_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[bass_boost_q_label] = widgetpos_temp
    bass_boost_q = DoubleVar(value=0.76)
    bass_boost_q_entry = Entry(
        canvas2,
        textvariable=bass_boost_q,
        width=7,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        bass_boost_q_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        tilt_label, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    ToolTip(
        tilt_label,
        "Target tilt in dB/octave. Positive value (upwards slope) will result in brighter frequency response and negative value (downwards slope) will result in darker frequency response.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        tilt_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        channel_balance_label, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        channel_balance_optionmenu, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[channel_balance_optionmenu] = widgetpos_temp
    ToolTip(
        channel_balance_label,
    ...
    channel_balance_db_entry = Entry(
        canvas2,
        textvariable=channel_balance_db,
        width=5,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        channel_balance_db_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[channel_balance_db_entry] = widgetpos_temp
    channel_balance_db_label = Label(canvas2, text="dB")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        channel_balance_db_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_label, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    ToolTip(
        decay_label,
        "Target decay time to reach -60 dB. When natural decay time is longer than the target decay time, a downward slope will be applied to decay tail. Decay cannot be increased with this. Can help reduce ringing in the room without having to do any physical room treatments.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_entry] = widgetpos_temp
    decay_per_channel = BooleanVar()
    decay_per_channel_checkbutton = Checkbutton(
        canvas2, text="per channel", variable=decay_per_channel, command=refresh2
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_per_channel_checkbutton, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_per_channel_checkbutton] = widgetpos_temp

    decay_fl_label = Label(canvas2, text="FL")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fl_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_fl_label] = widgetpos_temp
    decay_fl = Entry(
        canvas2, width=5, validate="key", vcmd=(root.register(validate_int), "%P")
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fl, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_fl] = widgetpos_temp
    decay_fc_label = Label(canvas2, text="FC")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fc_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fc, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_fc] = widgetpos_temp
    decay_fr_label = Label(canvas2, text="FR")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fr_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_fr, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_fr] = widgetpos_temp
    decay_sl_label = Label(canvas2, text="SL")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_sl_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_sl, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_sl] = widgetpos_temp
    decay_sr_label = Label(canvas2, text="SR")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_sr_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_sr, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_sr] = widgetpos_temp
    decay_bl_label = Label(canvas2, text="BL")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_bl_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_bl, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[decay_bl] = widgetpos_temp
    decay_br_label = Label(canvas2, text="BR")
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_br_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        decay_br, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        pre_response_label, pos2, imp_maxwidth, imp_maxheight
    )
    adv_options_pos[pre_response_label] = widgetpos_temp
    pre_response = DoubleVar(value=1.0)
    pre_response_entry = Entry(
        canvas2,
        textvariable=pre_response,
        width=7,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    ToolTip(
        pre_response_label,
        "Head room in milliseconds for cropping impulse response heads. Default is 1.0 ms.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        pre_response_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[pre_response_entry] = widgetpos_temp

    # JamesDSP
    jamesdsp = BooleanVar()
    jamesdsp_checkbutton = Checkbutton(
        canvas2, text="JamesDSP output", variable=jamesdsp
    )
    ToolTip(
        jamesdsp_checkbutton,
        "Generate true stereo IR file (jamesdsp.wav) for JamesDSP from FL/FR channels.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        jamesdsp_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    adv_options_pos[jamesdsp_checkbutton] = widgetpos_temp

    # Hangloose
    hangloose = BooleanVar()
    hangloose_checkbutton = Checkbutton(
        canvas2, text="Hangloose output", variable=hangloose
    )
    ToolTip(
        hangloose_checkbutton,
        "Generate separate stereo IR for each channel for Hangloose Convolver.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        hangloose_checkbutton, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[hangloose_checkbutton] = widgetpos_temp

    # Interactive plots
    interactive_plots = BooleanVar()
    interactive_plots_checkbutton = Checkbutton(
        canvas2, text="Interactive plots", variable=interactive_plots
    )
    ToolTip(
        interactive_plots_checkbutton, "Generate interactive Bokeh plots in HTML files."
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        interactive_plots_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    microphone_deviation_correction_checkbutton = Checkbutton(
        canvas2,
        text="Mic deviation correction",
        variable=microphone_deviation_correction,
        command=refresh2,
    )
    ToolTip(
        microphone_deviation_correction_checkbutton,
        "Enable microphone deviation correction to compensate for microphone placement variations between left and right ears.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        microphone_deviation_correction_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        mic_deviation_strength_label, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[mic_deviation_strength_label] = widgetpos_temp
    mic_deviation_strength = DoubleVar(value=0.7)
    mic_deviation_strength_entry = Entry(
        canvas2,
        textvariable=mic_deviation_strength,
        width=7,
        validate="key",
        vcmd=(root.register(validate_double), "%P"),
    )
    ToolTip(
        mic_deviation_strength_entry,
        "Microphone deviation correction strength (0.0-1.0). 0.0 = no correction, 1.0 = full correction.",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        mic_deviation_strength_entry, pos2, imp_maxwidth, imp_maxheight, samerow=True
    )
    adv_options_pos[mic_deviation_strength_entry] = widgetpos_temp

    # TrueHD 레이아웃 출력 옵션
    output_truehd_layouts = BooleanVar(value=False)
    output_truehd_layouts_checkbutton = Checkbutton(
        canvas2,
        text="TrueHD layouts (11ch/13ch)",
        variable=output_truehd_layouts,
        command=refresh2,
    )
    ToolTip(
        output_truehd_layouts_checkbutton,
        "Generate 11-channel (7.0.4) and 13-channel (7.0.6) layouts for TrueHD/Atmos content",
    )
    widgetpos_temp, pos2, imp_maxwidth, imp_maxheight = pack(
        output_truehd_layouts_checkbutton, pos2, imp_maxwidth, imp_maxheight
    )
    ...
    def impulcify():
        args = {
            "dir_path": dir_path.get(),
            "test_signal": test_signal.get(),
            "plot": plot.get(),
            "do_room_correction": do_room_correction.get(),
            "do_headphone_compensation": do_headphone_compensation.get(),
            "do_equalization": do_equalization.get(),
        }
        if do_room_correction.get():
            args["room_target"] = room_target.get() if room_target.get() else None
            args["room_mic_calibration"] = (
                room_mic_calibration.get() if room_mic_calibration.get() else None
            )
        ...
        if show_adv.get():
            ...
            args["target_level"] = (
                float(target_level.get()) if target_level.get() else None
            )
            args["channel_balance"] = (
                channel_balance_db.get()
                if channel_balance.get() == "number"
                else (
                    channel_balance.get() if channel_balance.get() != "none" else None
                )
            )
            ...
            if decay_per_channel.get():
                args["decay"] = {
                    decay_labels[i].cget("text"): float(decay_entries[i].get()) / 1000
                    for i in range(7)
                    if decay_entries[i].get()
                }
            elif decay.get():
                args["decay"] = {
                    decay_labels[i].cget("text"): float(decay.get()) / 1000
                    for i in range(7)
                }
            ...
            args["microphone_deviation_correction"] = (
                microphone_deviation_correction.get()
            )
    ...
    _, pos2, imp_maxwidth, imp_maxheight = pack(
        Button(canvas2, text="GENERATE", command=impulcify),
        pos2,
        imp_maxwidth,
        imp_maxheight,
    )

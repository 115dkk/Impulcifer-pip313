...
#### 🐛 버그 수정
- **휠의 RECORD가 들어 있는 파일과 맞지 않던 문제**: 3.x 휠에 넣는 `sweep-seg-FL,FR-stereo-6.15s-48000Hz-32bit-2.93Hz-24000Hz.wav`는 이름에 쉼표가 있는데, maturin(현재 최신인 1.15.0 포함)은 RECORD를 쓸 때 경로를 따옴표로 감싸지 않습니다. 그래서 RECORD를 CSV로 읽으면 이 줄은 `impulcifer/data/sweep-seg-FL`이라는 없는 파일이 되고, 실제 파일은 RECORD에 없는 것이 됩니다. pip는 설치할 때 'RECORD line has more than three elements' 경고를 냈고 `wheel unpack`은 실패했으며, PyPI는 3.0.3 Windows 휠을 두고 앞으로 이런 휠의 업로드를 받지 않겠다고 알려 왔습니다. 세 OS 휠이 모두 같은 상태였습니다(setuptools가 만드는 2.x 휠은 해당 없음). 이제 `crates/impulcifer-python/repair_record.py`가 빌드된 휠마다 RECORD를 CSV로 다시 쓰고(쉼표가 든 줄만 따옴표로 감싸고, 다른 항목의 바이트와 권한, 휠 파일의 수정 시각은 그대로 둡니다), 다시 읽어 모든 파일의 해시·크기와 맞춰 본 뒤 어긋나면 실패합니다. `release-3x.yml`과 `rust.yml`의 휠 잡, 로컬 `verify.py`가 maturin 빌드 직후 이 스크립트를 돌리고, `tests/migration/README-python.md`의 빌드 절차에도 이 명령을 넣었습니다. pip가 sdist나 체크아웃에서 휠을 빌드할 때도 같은 보정을 거치도록, 패키지의 빌드 백엔드를 maturin 훅을 그대로 넘기면서 `build_wheel`만 보정을 더하는 `build_backend.py`로 바꿨습니다(sdist는 `pyproject.toml`을 최상위로 옮기므로 두 스크립트를 sdist 최상위에도 넣습니다). 이미 올라간 3.0.3 이하의 휠은 PyPI 안내대로 그대로 둡니다.
- `tests/test_wheel_record_repair.py`가 maturin 방식의 RECORD로 만든 휠에서 재작성, 다른 항목 보존, 두 번 돌려도 같은 결과, 해시 불일치 검출, 빌드 백엔드의 `build_wheel` 보정과 `pyproject.toml` 설정을 확인하고, `crates/impulcifer-python/tests/test_native.py::test_wheel_record_matches_the_contents`가 CI에서 빌드한 실제 휠의 RECORD를 검사합니다.

## 3.0.3 - 2026-09-24
### Studio 고급 옵션을 켜고 끄는 탭 네 개로

#### ⭐ 개선
- **고급 옵션을 탭 네 개로 나누고 탭마다 켜기·끄기**: Studio 스킨의 고급 옵션은 모든 설정이 한 목록에 섞여 있었고, 스위치 하나가 전부를 켜거나 꺼서 목표 레벨만 쓰려 해도 저음 부스트 같은 값이 함께 적용될 수 있었습니다. 이제 설정을 음색·레벨, 시간 응답, 출력 파일, 보정·그래프 탭으로 나누고 탭마다 스위치를 두었습니다. 켠 탭의 값만 처리에 쓰고, 끈 탭의 값은 지우지 않고 잠가 둔 채 기본값으로 처리합니다. 탭 이름 앞의 점(채움 = 켜짐, 빈 원 = 꺼짐)으로 상태를 보여 주고, 탭마다 '기본값으로'로 그 탭의 값만 되돌립니다. 처음 실행하면 네 탭 모두 꺼져 있어 지금까지처럼 기본값으로 처리합니다.
- **적용할 수 없는 설정은 설명 대신 잠금**: 룸 보정, 헤드폰 보정, 사용자 EQ가 모두 꺼져 있으면 이퀄라이징 목표에 더하는 저음 부스트와 기울기를 잠급니다. 헤드폰 보정이 켜져 있으면 처리 때 건너뛰는 마이크 편차 보정을 잠그고 체크를 풀어 보여 주며, 헤드폰 보정을 끄면 원래 선택으로 돌아옵니다. 저음 부스트 게인이 0이면 Fc와 Q를, 채널별 감쇠를 켜면 전체 감쇠 값을 잠급니다. 잠긴 컨트롤에 마우스를 올리면 풀리는 조건을 짧게 보여 줍니다.
- **읽기 쉬운 입력칸**: 단위(dB, Hz, ms, dB/옥타브)를 라벨이 아니라 입력칸 안에 두고, 추가 출력 파일(JamesDSP, Hangloose, TrueHD 레이아웃)은 만들어질 파일 이름과 함께 고르며, 채널 밸런스는 trend·mids 같은 값 대신 '추세', '중역 레벨', '직접 입력' 같은 이름으로 고릅니다.
...
- **hrir.wav·hesuvi.wav 채널 순서 문서**: `docs/brir-channel-order.md`를 새로 쓰고 README의 추가 문서 목록과 무음 확장 채널 설명에서 연결했습니다. 스피커 하나가 왼쪽 귀·오른쪽 귀 트랙 두 개라는 점, 트랙 이름의 `left`/`right`가 스피커 쪽이 아니라 소리를 듣는 귀라는 점을 먼저 짚고, 두 파일의 모든 트랙(hrir.wav 32개, hesuvi.wav 30개)을 WAV 번호(1부터)·인덱스(0부터)·스피커·귀·같은 쪽/반대쪽으로 표에 적었습니다. hesuvi.wav에서 FC가 7번과 14번으로 떨어져 있고 FR·SR·BR은 오른쪽 귀 트랙이 먼저 온다는 점, HeSuVi 가상 채널 이름(`R1` 등)의 글자는 스피커가 아니라 귀를 가리킨다는 점, 두 파일 모두 홀수 번호가 왼쪽 귀라는 점 등 헷갈리기 쉬운 대목과, 데모 출력의 도착 시각·레벨로 귀 배정을 확인하는 방법도 담았습니다. 표는 `tests/test_channel_order_doc.py`가 `core/constants.py`의 순서와 한 줄씩 대조합니다.
...
#### ⭐ 새로운 기능 / 개선
- **호스트 API 자동 선택이 WASAPI를 먼저 고릅니다**: 장치 이름에 호스트 API를 붙이지 않고 `--host_api`도 주지 않으면 지금까지 DirectSound → MME → WASAPI 순으로 장치를 찾았습니다. DirectSound와 MME는 Vista 이후 WASAPI 위의 에뮬레이션이라 믹서 단계만 하나 더 거치고(MME는 장치 이름을 31자에서 자릅니다) WASAPI에 없는 기능이 없습니다. 이제 WASAPI → DirectSound → MME 순입니다. CTk Stable/Studio 레코더 탭의 기본 호스트 API도 `Windows WASAPI`로 바꿨습니다.
...
- **`impulcifer-dsp` 3차 프리미티브 (P06)**: nnresample 0.2.4.1의 32001탭 널-온-나이퀴스트 설계(2^19 FFT 탐색, 설계당 13 ms 이내), scipy `resample_poly`의 패딩 산술을 그대로 따르는 polyphase 리샘플링(업샘플 신호를 만들지 않음), `nnresample(x, new_fs, old_fs)`와 설계 캐시, 2.x 플로터의 nfft/noverlap 선택 규칙과 scipy `spectrogram(mode="psd")`. 골든 대비 탭 오차 2.4e-15, 리샘플 출력 2.3e-17. 설치본 nnresample은 같은 레이트 설계에서 예외를 내므로 그대로 두었다(2.x 파이프라인은 같은 레이트에서 리샘플 단계를 건너뛴다).
- **`impulcifer-dsp::fr` AutoEQ FrequencyResponse 서브셋 (P09)**: 2.x 코어가 쓰는 메서드만 포팅했다. `generate_frequencies`(반복 곱셈 그리드), `interpolate`(log10 축 FITPACK, 0 Hz 보정), `center`/`center_value`, `create_target`(44100 Hz 설계 버릇 유지), `compensate`, `smoothen`·`smoothen_fractional_octave`·`smoothen_heavy_light`, `equalize`(시그모이드 상한, kink 재보간), `minimum_phase_impulse_response`, `read_from_csv`(정규식 버릇 포함), `magnitude_to_frequency_response`. 골든 69개, 테스트 18개. 최소위상 FIR 체인은 파이썬 자체가 1e-12 섭동에 1e-3 dB(가청 대역)·9 dB(나이퀴스트 근방)까지 흔들리는 것을 `tests/migration/oracle_noise_minimum_phase.py`로 실측해, 그 잡음 한계에 여유를 둔 대역별 허용오차로 고정했다(Rust 오차는 그 안에 있다). 조사 문서 `docs/rust/survey/`와 작업서 P08·P09·PA(성능 감사) 추가.
- **`impulcifer-io` 성능 감사 (PA01)**: 같은 머신에서 soundfile/`core.audio_io`와 비교한 첫 감사. 첫 측정에서는 9개 연산 중 8개가 파이썬보다 느렸다(샘플마다 `write_all`·인코딩 분기, 샘플 단위 `read_exact`). 블록 단위 입출력, 폭이 고정된 PCM 인코딩, 2^52 가감으로 짝수 반올림을 유지하는 양자화, 30/32트랙 파일의 4스레드 디코드로 고친 뒤 CPython 3.14.5와 free-threaded 3.14.7t 모두에 대해 9개 연산 전부가 1.18배에서 4.79배 빠르다(`docs/rust/perf/impulcifer-io.md`). 골든은 바이트 단위로 그대로 통과하며, 등록부의 `perf.impulcifer-io`가 `bench_smoke_impulcifer_io`를 인용한다.
- **`impulcifer-dsp` 측정 객체 (P08)**: 파일 입출력 없는 `SweepEstimator`(Farina 스윕·역필터·`estimate`·`sweep_sequence`·`from_wav` 복원 규칙), `ImpulseResponse`(shift·crop·equalize·resample·magnitude_response), `decay`(Lundeby 파라미터·Schroeder 감쇠 시간·감쇠창), `Hrir`(녹음 분할의 2.x 규칙과 버릇, crop_heads·양측/온셋 정렬·crop_tails·normalize·트랙 행렬)와 `core/constants.py`의 트랙 순서 상수. 데모 녹음을 열어 crop_heads→정렬→crop_tails→normalize까지 단계별로 파이썬과 비교하는 프로토콜 골든(35 MB, 예산 40 MB)으로 고정했고, 생성한 스윕은 번들 WAV와 PCM_32 불일치 0개다. 골든 26개 테스트, 최대 오차 1e-11 dB(magnitude), 그 외 1e-15 이하.
- **`impulcifer-dsp` 성능 감사 (PA02)**: 프리미티브 22개 연산을 CPython 3.14.5(NumPy 2.5.3, SciPy 1.18.1)의 scipy/numpy/nnresample과 같은 머신에서 비교했다. FFT 계획과 작업 버퍼의 스레드별 재사용, 컨볼루션 복사 제거, polyphase 계수의 연속 배치, SOS 4섹션 처리, minimum phase의 실수 FFT, Kaiser 창 대칭 재사용으로 22개 전부 파이썬보다 빠르다(full 컨볼루션 1.80배, polyphase 1.69배, spectrogram 2.95배, 최소 `expit` 1.14배). 중간 재측정에서 magnitude response가 파이썬보다 느렸던 것을 버리지 않고 추가로 고쳤다(0.41 ms 대 1.44 ms). 골든 허용오차와 공개 시그니처는 그대로이며 `perf.impulcifer-dsp`가 `bench_smoke_impulcifer_dsp`를 인용한다.
- **`impulcifer-dsp` 스테이지 헬퍼와 인메모리 파이프라인 (P10)**: 룸 보정(개별·generic, 제한 마스크, 44100 Hz 셸프 등 2.x 버릇 유지), 헤드폰 보상과 파일 선택 규칙, EQ 파일(CSV, 우선순위; EqualizerAPO는 P12), 타깃, 스피커별 EQ FIR 체인(rayon), 감쇠 보정, 채널 밸런스 7종, 가상 저음, 마이크 편차 보정, README 수치, 그리고 26행 스테이지 표를 그대로 옮긴 `run_pipeline`(진행률·취소 옵저버, hrir/hesuvi/TrueHD/JamesDSP/Hangloose 트랙 행렬). 데모에서 기본·vbass·채널 밸런스·마이크 편차·감쇠·리샘플·옵션 6종·generic room 시나리오 골든 795개(21.5 MB, 작업서 예외 25 MB)와 테스트 24개. 최소위상 EQ FIR 잡음이 정규화 게인(2.7e-6 dB)·PNR(0.03 dB)·감쇠 무릎(2표본)로 전파되는 것을 고정 FIR 격리 테스트로 확인하고, 그 하류 항목의 허용오차를 관측치의 5배로 잡았다.
- **`impulcifer-dsp::stages::eqapo` EqualizerAPO 설정 파서 (P12)**: `core/eqapo.py`를 그대로 옮겼다. 바이쿼드 14종(slope/Q/BW, 기본값, REW 표기, 소수점 콤마), IIR, GraphicEQ, Preamp, Channel 범위, If/ElseIf/Else 조건, Include(깊이 8, 순환 검사), Convolution(파일은 `EqApoLoader` 트레이트로 호출자가 공급). 골든 코퍼스 70여 건에서 dB 오차 0(IIR·FIR 1e-13), 적용·우회·건너뜀 보고 문자열은 파이썬과 원문 일치. `stages::eq_files::read_eq_settings`가 평문 CSV와 EqualizerAPO 본문을 모두 처리한다.
- **`impulcifer-service` BRIR 완주 (P11, M2)**: 측정 폴더 검색, 스윕 별칭·`generate:` 사양·`test.wav`·자동 감지, 룸·헤드폰·EQ 입력 로딩, `run_pipeline` 실행, `responses.wav`·`README.md`(i18n 렌더링)·`hrir.wav`·`hesuvi.wav`(ICHL 맵 포함)·TrueHD·JamesDSP·Hangloose 쓰기, 그리고 `start_brir`·`detect_sweep`·`generate_sweep_set` IPC. 데모 기본·vbass 시나리오에서 2.x가 쓴 WAV와 표본 오차 1e-6 이내(피크 인덱스·길이·트랙 이름 정확 일치), 진행률 이벤트는 2.x의 `cli_*` 키와 11단계를 그대로 낸다. README는 문구·표 구조를 바이트 단위로 대조하고 숫자만 감쇠 잡음 예산(기본 0.1 dB·2 ms, vbass는 꼬리가 1e-8 수준의 필터 잔향이라 5 dB·20 ms)으로 비교한다. 3.x 전용 문구는 서비스 오버레이 카탈로그에 두어 2.x 카탈로그와 릴리스 게이트를 건드리지 않는다. `impulcifer-io::brir_layout`(ICHL 청크) 추가.
- **`impulcifer-service` 녹음 완주 (P16)**: `start_recording`이 기존 IPC 그대로 돕니다. 2.x의 요청 검증 순서·오류 코드·문구(골든 136건 + 스윕 86건), 즉석 스윕 생성(PCM_32 왕복으로 파이썬과 SHA-256 동일), 실제 백엔드(WASAPI/cpal) 장치 열거와 이름 해석, exclusive 우선·shared 폴백, 재생 길이와 같은 표본 수의 PCM_32 캡처, append(짧은 쪽 0 채움·새 트랙 추가), `test.wav` 사이드카 규칙, 열 개 필드의 진행 이벤트(0.98/0.99/1.0 사다리), `analyze_recording` 요약. 2.x처럼 취소는 없다. CABLE-A 가상 케이블 실기 테스트에서 878,540프레임을 녹음했고 `detect_sweep`이 기본 스윕을 높은 신뢰도로 찾았다. 백엔드는 서비스가 `Arc`로 공유해 주입한 가짜 백엔드와 실기가 같은 경로를 탄다.
- **`impulcifer-cli` 완성 (P13)**: `impulcifer --dir_path ...`가 3.x에서도 2.x와 같은 옵션으로 돕니다. 옵션 33개의 이름·순서·도움말·choices·기본값을 실제 argparse 파서에서 뽑은 골든과 대조하고, `--bass_boost`(값 하나 또는 셋)와 `--decay`(공통 ms 또는 `FL:300,FR:250`), `--c`→`head_ms`, `--no_*` 플래그, 생략된 기본값을 2.x의 `create_cli()`와 같은 kwargs로 만듭니다(argv 12묶음 골든). 알 수 없는 옵션·값 누락·잘못된 choice는 argparse처럼 usage 줄과 `impulcifer: error:`를 stderr에 쓰고 2로 끝나며, `--version`/`--info`/`-h`는 0으로 끝납니다. 실행은 서비스 계층의 작업 등록부를 100 ms로 폴링해 2.x 콘솔 로거의 접두사(✓ ✗ ⚠)와 `[NN%]` 진행 줄을 그대로 찍고, 끝에 README를 출력합니다. 언어는 `~/.impulcifer/settings.json`을 읽기만 해서 고르고(서비스의 `settings::saved_language`·`settings::catalog`), 데이터 디렉터리도 서비스의 `default_data_dir`을 그대로 씁니다. Ctrl-C 처리는 아직 없습니다. `pub fn run(argv, out, err) -> i32`라서 P14의 PyPI wheel이 프로세스 없이 같은 CLI를 부릅니다.
- **`impulcifer-service` 출력 복원 (P17)**: `start_output_recovery`가 3.x에서도 돕니다. `core/brir_recovery.py`의 소스 우선순위(hrir+hesuvi 교차 검증, hrir, hesuvi, Hangloose), 대소문자를 가리지 않는 파일 찾기와 모호성 오류, ICHL 채널 맵, 위치 기반 채널 수 검증, 비무음 LFE 거부, Hangloose 분할 파일 매칭(가장 긴 스피커 접미사, 공통 접두사), 무음 채널 제거, PCM_32 출력과 ICHL 청크, 기존 파일 보존과 실패 시 정리를 그대로 옮겼습니다. 시나리오 97개에서 만든 파일 76개가 파이썬과 바이트 단위로 같고 오류 봉투 52개가 일치합니다(손상된 WAV의 원인 문구만 libsndfile과 다름). 파일 게시는 하드 링크로 기존 파일을 원자적으로 거부하고, 하드 링크가 없는 파일시스템(FAT/exFAT 등)에서는 파이썬과 같은 rename으로 물러납니다. 파이썬이 날 `OSError`로 내보내던 경우는 `OUTPUT_WRITE_FAILED`에 OS 사유를 담아 돌려줍니다.
- **파이프라인 전체 성능 감사 1차 (PA03, `docs/rust/perf/pipeline-demo.md`)**: 데모 BRIR 전체 실행을 같은 머신(i5-12600KF)에서 2.x와 비교했다. 프로세스 전체 중앙값은 Rust 1.2~1.6초, CPython 3.14.5 13.0~13.4초(8.4~10.6배), free-threaded 3.14.7t 5.8~5.9초(3.7~4.8배)이고, DSP 코드는 손대지 않았다. 최대 메모리는 Rust 프로세스 트리 260 MiB, 일반 파이썬 트리 913~947 MiB(EQ용 프로세스 풀 포함), 3.14t 526 MiB로, 스레드 기반 3.14t가 일반 파이썬보다 트리 RSS를 42~45% 덜 쓴다는 것을 확인했다. 다만 파이썬은 기본 실행에서 PNG 플롯 두 장을 더 만들고 Rust는 아직 플롯이 없어 같은 산출물끼리의 비교가 아니므로 `perf.pipeline-demo`는 `planned`로 두고 플롯 구현 뒤 다시 잰다. 측정 도구는 `examples/demo_brir.rs`, `benches/perf.rs`, `tests/perf_smoke.rs`(CI에서 도는 `--fast` 스모크), `tests/migration/bench_oracle_pipeline.py`다.
- **PyPI wheel `impulcifer` (P14, `crates/impulcifer-python`)**: PyO3 0.27 + maturin로 만드는 3.x 배포판입니다. `impulcifer.main(**kwargs)`가 2.x와 같은 kwargs 관례로 서비스 계층의 BRIR 작업을 돌리고(처리 중 GIL 해제, 진행·로그 콜백은 이벤트마다 인터프리터에 붙어 호출, 서비스 오류는 코드를 담은 `RuntimeError`), `detect_sweep`·`generate_sweep_set`·`recover_brir_outputs`와 `impulcifer` 콘솔 스크립트(P13의 CLI를 파이썬 stdout/stderr로 실행)를 제공합니다. abi3-py39 wheel 하나로 CPython 3.9 이상을 덮고, free-threaded 3.14t는 PyO3가 abi3를 무시하므로 별도 wheel을 만듭니다(모듈은 `gil_used = false`). 번들 스윕과 Harman 타깃은 `sync_data.py`가 빌드 직전에 `data/`에서 복사해 넣고 패키지가 `IMPULCIFER_DATA_DIR`로 가리킵니다(복사본은 저장소에 넣지 않음). 설치된 wheel을 실제로 import해 데모를 돌리는 pytest 14개가 3.14와 3.14t에서 통과했고, `rust.yml`의 `wheel` 잡이 Ubuntu·Windows에서 같은 검사를 돕니다. 등록부 게이트는 이제 `pytest:<파일>::<함수>` 참조도 검사합니다.
- **서비스의 셸 서브프로세스 제거 (P18)**: README 시각, 첫 실행 언어 감지, OS 설명을 위해 PowerShell·cmd.exe·date·uname·locale을 띄우던 코드를 없앴습니다. 시각은 chrono, 언어는 sys-locale, OS 설명은 Windows에서 os_info(레지스트리), Linux에서 `/proc/sys/kernel/osrelease`, macOS에서 `SystemVersion.plist`를 직접 읽습니다. Tauri 앱에서 콘솔 창이 깜빡이거나 실행마다 100 ms 이상 걸리던 원인이었습니다. 정책 게이트 `no_shell_subprocesses`가 `std::process`를 쓰는 파일의 `Command::new(`를 ffmpeg 모듈 밖에서 금지합니다.
- **Tauri 앱 M3 스모크 (P15)**: 2.x 프론트엔드를 한 바이트도 바꾸지 않은 채 Tauri 셸 위에서 BRIR 생성, 출력 복원, 언어 전환(설정 저장 확인), 업데이트 미구현 봉투 표시, CABLE-A 헤드폰 녹음이 실제 UI 조작으로 끝까지 돕니다(`docs/rust/APP-SMOKE.md`, 화면 캡처 포함). WebView2 152 런타임이 원격 디버깅 포트를 열지 않아 CDP 대신 스모크 모드(`IMPULCIFER_APP_SMOKE=1`)에서 앱이 드라이버 스크립트를 주입하고 결과를 기존 `pywebview_api` 커맨드의 `smoke_report` 메서드로 파일에 쓰는 방식을 씁니다. 스모크에서 드러난 셸 결함 두 가지도 고쳤습니다. 프론트엔드가 참조하는 로고 두 장이 `frontendDist` 밖에 있어 리소스 오류가 나던 것을 셸이 같은 URL로 내주고, Tauri dialog 플러그인이 `window.confirm`을 비동기로 바꿔치기해 사용자 확인 전에 녹음이 시작되던 것을 동기 confirm을 유지하는 플러그인 래퍼로 막았습니다.
- **3.x 릴리스 워크플로 (P22, `release-3x.yml`)**: 수동 실행이나 `v3.*` 태그로만 도는 별도 파이프라인입니다. Windows는 Velopack 패키징 뒤 일회용 러너에서 최신 2.x Setup을 설치하고 3.x 패키지를 `Update.exe apply`로 적용하는데, 이때 업데이터가 앱을 다시 띄우는 정상 경로를 그대로 쓰고 스모크 하네스가 그 프로세스에 붙어 검증합니다(`smoke.py --launch-command`). macOS dmg와 Linux AppImage는 서명된 업데이터 아티팩트와 함께 만들고, wheel·sdist를 세 OS에서 빌드하며(`publish_pypi` 입력일 때만 PyPI 발행), `latest.json`·`SHA256SUMS.txt`를 붙인 GitHub Release를 만듭니다(버전에 접미사가 있으면 prerelease). Codex 리뷰(#197) 반영으로, 사전 출시 설치본이 다음 사전 출시를 볼 수 있도록 롤링 `updater-3x-pre` 릴리스에 `latest.json`을 갱신하고 앱은 자기 버전이 사전 출시일 때 그 피드를 먼저 읽으며(Windows는 Velopack GithubSource로 GitHub API에 prerelease 포함 조회), ALSA에 링크되는 Linux wheel은 manylinux를 주장하지 않고 `linux_x86_64`로 GitHub Release에만 올리고(Linux pip는 sdist), 수동 실행에서 태그가 없으면 체크아웃한 커밋에 만들며, `system` 테마는 OS 설정을 읽어 첫 프레임 배경색을 정합니다.
- **M5(출시 전 성능 감사) 승인**: 소유자가 2026-09-09에 M5를 통과로 승인했습니다. 플롯 포함 파이프라인은 CPython 3.14.5보다 11.6~12.0배, 3.14.7t보다 5.0~5.1배 빠르고 메모리는 224~234 MiB 대 915~946/525 MiB이며, io·dsp·service의 모든 연산이 파이썬 이상입니다. 오디오 헤드폰 스윕 전체 시간이 PortAudio보다 15~17 ms(0.25~0.33%) 긴 것은 40 ms shared 버퍼의 대가로 명시적 예외로 승인했고, 등록부의 `perf.release`·`perf.impulcifer-audio-io`·`perf.impulcifer-sys-win`을 구현으로 바꿨습니다. 3.x 첫 릴리스에 남은 것은 PyPI Trusted Publisher 설정과 `release-3x.yml` 수동 실행(2.x→3.x 업그레이드 러너 검증)입니다.
- **3.0.0-alpha.0 첫 사전 출시 (2026-09-09)**: `release-3x.yml`이 처음으로 끝까지 돌았습니다. GitHub 사전 출시 v3.0.0-alpha.0(Velopack Setup·Portable·nupkg와 `releases.win.json`, macOS dmg와 서명된 업데이터 아카이브, Linux AppImage와 서명, wheel 세 개와 sdist, `latest.json`, `SHA256SUMS.txt`), 사전 출시 설치본용 롤링 피드 `updater-3x-pre`, PyPI `impulcifer-py313` 3.0.0a0(사전 출시라 `--pre`로만 설치). 일회용 러너에서 2.x v2.14.1을 설치하고 3.x를 `Update.exe`로 적용해 업데이터가 앱을 다시 띄운 뒤 스모크 하네스가 붙어 검증했으므로 `package.upgrade_from_2x`를 구현으로 바꿨습니다. `/releases/latest`는 여전히 2.x 정식(v2.14.1)을 가리킵니다.
- **3.x wheel은 2.x와 같은 PyPI 프로젝트 `impulcifer-py313`으로 발행**: 새 프로젝트 `impulcifer`를 따로 파면 기존 설치본·업데이터 피드·PyPI 의존이 갈라져 사용자가 이주 비용을 치르게 되므로, 3.x wheel의 배포 이름을 `impulcifer-py313`으로 되돌렸습니다. 정식 3.x가 나오면 `pip install --upgrade impulcifer-py313`으로 넘어가고, alpha는 `--pre`로만 받습니다. PyPI 쪽에는 같은 프로젝트에 `release-3x.yml`(환경 `PyPI`)용 Trusted Publisher를 하나 더 두면 됩니다.
- **3.0.0-alpha.1: 알파를 벗어나기 전 정리(ADR 0003)**: 3.x 화면을 `apps/impulcifer-app/ui/`로 분기해 2.x의 `webview_ui/`·`i18n/locales/`는 그대로 두고, 3.x 문자열은 `crates/impulcifer-service/locales/`의 아홉 언어 오버레이(키·자리표시자 동일성 검사, 화면 키 해석 검사)에 둡니다. Windows 릴리스 빌드는 GUI 서브시스템이라 앱 옆에 콘솔 창이 뜨지 않고(릴리스 워크플로가 PE 서브시스템을 검사), 페이지·초기화 스크립트·스모크 드라이버는 `// @ts-check`와 `ui/ipc.d.ts`의 IPC 선언으로 `tsc --checkJs`(strict)를 오류 0으로 통과하며 `rust.yml`의 `js` 잡과 정책 게이트가 이를 지킵니다.
- **녹음 장치 접근 방식 선택 (`share_mode`)**: 녹음 화면의 '장치 접근 방식'에서 자동(독점 우선, 거부되면 공유)·독점·공유를 고릅니다. 독점이나 공유를 고정하면 폴백 없이 한 번만 열고, 장치가 거부하면 어느 모드가 왜 거부됐는지(`share_mode_refused`) 그대로 알립니다. cpal 백엔드(macOS·Linux)는 고정 선택을 받지 않아 선택이 비활성화됩니다. CABLE-A 실측: 자동은 공유로 폴백해 성공, 독점 고정은 형식 거부로 실패, 공유 고정은 성공. 성공하면 실제로 열린 출력·입력 모드를 표시합니다. 호스트 API 행은 둘 이상일 때만 보입니다.
- **Studio 복원 화면 재설계**: 폴더나 옵션을 바꾸면 새 IPC `plan_output_recovery`(실제 복원과 같은 준비 단계를 쓰는 읽기 전용 드라이런)로 원본 종류·샘플레이트·길이·스피커와 파일 장부(있음/만들 예정/생성 완료)를 먼저 보여 주고, 계획이 서야 복원 버튼이 켜집니다. Stable 스킨은 기존 폼과 모달을 유지합니다.
- **러스트판 설정·정보 화면**: CustomTkinter 선택 항목과 Python·GIL 표시를 없애고, 설정은 화면 모양(스킨·테마·언어)과 폴더(데이터 폴더·설정 파일 경로 열기) 두 카드로, 정보는 Rust 툴체인·Tauri·WebView 엔진·오디오 백엔드·OS·CPU·업데이트 채널·경로를 보여 줍니다(`get_system_info`의 러스트판 응답). 2.x가 `frontendDist` 밖에서 참조하던 Pretendard·JetBrains Mono 글꼴을 3.x 화면에 동봉했습니다.
- **3.0.0-alpha.2: 사전 출시 설치본의 업데이트 확인이 다음 alpha를 봅니다**: v3.0.0-alpha.1은 발행됐지만 alpha.0 앱의 '업데이트 확인'은 "이미 최신 버전입니다 (최신: 2.14.1)"라고 답했습니다. 앱 안의 확인(`check_for_updates`)이 설치 종류와 무관하게 GitHub `/releases/latest`만 읽었고, 그 엔드포인트는 사전 출시를 절대 돌려주지 않기 때문입니다. 이제 사전 출시 설치본은 릴리스 목록(`/releases?per_page=30`)을 읽어 초안과 `updater-3x-pre` 같은 버전 없는 태그를 건너뛰고 가장 높은 PEP 440 버전을 고르므로 다음 alpha와 그 뒤의 정식을 모두 봅니다. 정식 설치본은 그대로 `/releases/latest`만 봅니다. `latest_version`은 사전 출시 구간(`3.0.0-alpha.1`)을 유지해 표시합니다. macOS·Linux의 Tauri 업데이터가 롤링 피드에서 받은 버전이 사용자가 확인한 버전과 같은지도 사전 출시 구간까지 비교하므로, 릴리스 직후 피드가 아직 앞 alpha를 가리키는 순간에 다른 판이 설치되지 않습니다(Codex #207). alpha.0·alpha.1 설치본은 옛 코드가 돌고 있으므로 한 번은 손으로(Setup.exe·dmg·AppImage) 올려야 하고 그다음부터 확인이 동작합니다.
- **헤드폰 차트에 실제 적용된 보정 곡선**: `plots/headphones.png`가 측정값과 목표만 보여 주던 것을, 이퀄라이즈 단계가 끝나면 파이프라인이 실제로 FIR로 만든 보정 곡선(앞왼쪽 스피커의 왼쪽 귀, 앞오른쪽 스피커의 오른쪽 귀)을 넘겨받아 보라색 파선으로 다시 그립니다. 제목이 `Your headphones and the correction applied`로 바뀌고 세 번째 요약문에 40 Hz–16 kHz의 보정 범위와 가장 큰 값·주파수가 붙습니다. 보정이 없으면 기존 그림 그대로입니다. DSP는 `StageObserver::on_equalized` 훅으로 귀별 보정 곡선을 넘길 뿐 FIR 바이트와 WAV·README·골든은 그대로입니다(`docs/rust/PLOTS.md`).
- **Python 휠에서 오디오 백엔드를 뺐고 Linux 휠이 manylinux_2_28로 PyPI에 올라갑니다**: Python 표면(`run`·`detect_sweep`·`generate_sweep_set`·`recover_brir_outputs`·`cli_main`)에는 녹음 진입점이 없는데도 서비스가 cpal(ALSA)을 링크해 Linux 휠은 `linux_x86_64`로만 나와 PyPI에 올릴 수 없었습니다. 오디오 백엔드를 `impulcifer-audio-io`의 `native`, 서비스·CLI의 `native-audio` 기능(기본 켜짐)으로 묶고 Python 크레이트는 이를 끄므로(`NullBackend`: 장치 없음, `audio_backend = "none"`, 녹음은 `DEVICE_ERROR`), Linux 휠은 `PyO3/maturin-action`의 manylinux_2_28 컨테이너에서 `--auditwheel check`로 빌드되어 외부 라이브러리가 하나라도 링크되면 빌드가 실패합니다. 세 플랫폼 휠이 모두 PyPI로 갑니다. 정책 게이트가 Python 크레이트의 `default-features = false`를 검사합니다.
- **오디오 버퍼 재측정 (PA07)**: alpha.2 오디오 스택(share_mode 선택이 들어간 뒤)에서 CABLE-A로 공유 모드 버퍼를 다시 쟀습니다. 2주기(1056프레임)와 3주기(1440프레임)는 헤드폰 스윕과 7스피커 세트 각각 10회 모두 underrun이 있어 기본값 `SHARED_BUFFER_PERIODS = 4`(1920프레임, 40 ms)를 유지합니다. 회귀 표에서 헤드폰 스윕 wall time은 PortAudio 명시적 스트림보다 14.2~15.2 ms(0.23~0.25%) 느려 M5에서 승인한 예외 범위 안이고, 캐시 열거 76배·duplex open/close 1.27~1.33배는 그대로입니다. 런타임 코드는 바꾸지 않았고 보고서는 `docs/rust/perf/impulcifer-audio-io.md`의 여덟 번째 실행입니다.
- **사람을 위한 플롯 (P23)**: 3.x의 플롯은 더 이상 matplotlib 그림을 흉내 내지 않습니다. 종이 느낌의 밝은 한 가지 스타일에 큰 글자, 차트마다 쉬운 제목과 한 줄 설명, 주파수 축 위의 대역 띠(서브베이스·베이스·중역·프레즌스·에어), 그리고 그림 위에 적힌 핵심 숫자(좌우 평균 차이와 최대 차이, 방의 최대 피크·딥 위치, 잔향 길이, 첫 반사음의 시각과 크기, 어느 귀에 먼저 닿는지와 몇 ms인지)로 바꿨습니다. 여섯 패널 시트의 워터폴은 임펄스 응답 포락선으로 첫 반사음을 보여 주는 패널로, 시트 제목은 `Front left speaker → left ear · after processing`처럼 풀어 쓴 문장으로 바꿨습니다. 헤드폰 차트는 측정값과 목표만 보여 주고 적용 EQ는 파이프라인이 넘겨줄 때 추가합니다. 그림 뒤의 숫자(시리즈 골든 테스트)와 파일 이름은 그대로이며, 시각 체계는 `docs/rust/PLOTS.md`에 있습니다.
- **다크 제목표시줄**: 앱 창을 만들 때부터 저장된 테마와 Pulse 배경색(`--bg-0`)을 지정해, 2.x pywebview의 DWM 우회처럼 첫 프레임부터 제목표시줄과 로딩 전 배경이 UI 테마를 따릅니다. DWM의 immersive dark mode 속성이 dark 테마에서 1, light 테마에서 0으로 읽히는 것을 실기로 확인했습니다.
- **플롯 출력 `impulcifer-plots` (P19)와 M2 성능 게이트 확정**: 파이프라인이 만드는 PNG 전부를 Rust에서 그립니다(plotters + 내장 DejaVu Sans, 시스템 폰트 없음). 기본 실행의 `results.png`·`plots/headphones.png`, `--plot`의 스피커별 pre/post 패널(2x3, 두 단계 축 동기화), 룸 보정 플롯, 인터오럴 오버레이, EQ 플롯까지 2.x와 같은 파일 집합(기본 2개, `--plot` 51개)을 matplotlib 캔버스 크기 그대로 만듭니다. 플롯 단계가 리샘플링 전에 돌기 때문에 `StageObserver::on_plot` 훅으로 그 시점의 HRIR을 서비스에 넘깁니다. 그려지는 데이터는 파이프라인 결과끼리 0.05 dB 이내(실측 최대 1.7e-2 dB), 같은 입력의 smoothing은 1e-13 dB 수준으로 파이썬과 같고, `--plot` 유무에 따라 WAV와 README 바이트는 변하지 않습니다. tight-bbox 자르기, 60색 양자화, 3D waterfall(2D 히트맵으로 대체), Bokeh 대화형 플롯, 마이크 편차 디버그 플롯은 재현하지 않았습니다. 플롯까지 포함한 재측정에서 데모 BRIR 전체 실행이 CPython 3.14.5보다 11.9배, free-threaded 3.14.7t보다 5.2~5.5배 빠르고 프로세스 트리 최대 RSS는 233 MiB 대 913~946 MiB 대 526 MiB라 `perf.pipeline-demo`를 구현 완료로 등록했습니다.
- **업데이터 (P20)**: `check_for_updates`·`start_update`·`apply_pending_update`가 3.x에서도 2.x와 같은 필드로 돕니다. 업데이트 확인은 GitHub `releases/latest`를 ureq(rustls)로 10초 안에 읽어 플랫폼별 설치 파일 선택 규칙과 버전 비교를 2.x와 맞췄습니다(골든 30건). 실행은 설치 종류에 따라 갈립니다. Windows Velopack 설치는 Velopack Rust SDK로 내려받고 적용 시 `Update.exe`에 넘겨 재시작하며, macOS·Linux 번들은 Tauri 업데이터(서명된 `latest.json`)를 앱의 호스트 어댑터로 부르고, 그 밖(개발 실행, 맞지 않는 번들)은 2.x의 legacy 방식대로 임시 폴더에 내려받아 SHA256SUMS를 검증한 뒤 설치 파일을 엽니다(AppImage는 제자리 교체). `bootstrap`과 `get_system_info`의 `install_kind`가 실제 판정값을 돌려줍니다. 실제 설치본에서의 적용·재시작 검증과 피드·`latest.json` 발행은 패키징 작업에서 합니다. Codex 리뷰(#194) 반영으로, 기본 버전이 같을 때는 PEP 440 사전 출시 순서를 비교해 alpha 설치가 rc·정식을 업데이트로 보고, 설치 파일 다운로드는 본문에 전역 제한 시간을 두지 않으며, Velopack 실행기는 로컬 피드와 시뮬레이션 설치 디렉터리에서 내려받기·스테이징·적용 실패 처리까지 테스트로 검증합니다.
- **패키징 (P21)**: Tauri 번들 설정(macOS app+dmg, Linux AppImage, Windows는 Velopack이 실행 파일을 그대로 포장, 번들 스윕과 Harman 타깃을 `resources/data`에, 업데이터 아티팩트와 아이콘 세트)과 Windows용 `build_scripts/pack_velopack.ps1`(release 빌드, 스테이징, `vpk pack`, 2.x 설치본과 같은 pack id `Impulcifer`)을 넣었습니다. 로컬에서 `Impulcifer-win-Setup.exe`·full nupkg·`releases.win.json`을 만들고, 사용자의 실제 설치본은 건드리지 않은 채 임시 2.13.3 locator로 Velopack SDK의 확인·다운로드·적용 계획을 검증했습니다(실제 적용·재시작은 릴리스 워크플로의 Windows 러너에서). 번들 안에서 데이터 디렉터리는 앱이 `with_dependencies`로 서비스에 넘깁니다. 플랫폼별 명령과 `latest.json` 형식, PyPI 프로젝트 `impulcifer`의 Trusted Publisher 요건은 `docs/rust/PACKAGING.md`에 있습니다.
- **서비스 계층 성능 감사 (PA04, `docs/rust/perf/impulcifer-service.md`)**: IPC 호출과 작업 이벤트 처리 9개 연산을 2.x `ImpulciferService`와 같은 머신에서 비교해 전부 파이썬보다 빠릅니다(CPython 3.14.5 대비 1.5~15.6배, 3.14.7t 대비 1.6~16.6배). 내장 카탈로그를 언어별로 한 번만 파싱하고, IPC 응답과 이벤트 payload의 불필요한 복사를 없앴으며, `detect_sweep`은 WAV 헤더로 길이를 먼저 판단해 파이썬과 같은 조건에서만 샘플을 디코딩하고(13.2 ms → 0.28 ms), 카탈로그 치환은 한 번에 훑습니다(출력 바이트 동일). 임시 입력을 지우기 전에 작업 스레드를 기다리는 `JobRegistry::join`도 더했습니다.
- **오디오 계층 성능 감사 (PA05, `docs/rust/perf/impulcifer-audio-io.md`)**: CABLE-A 가상 케이블에서 여섯 번에 걸쳐 2.x(sounddevice/PortAudio)와 비교했습니다. 장치 열거는 2초 캐시로 76배, 세션 열기·닫기는 렌더·캡처 클라이언트 동시 초기화로 1.28배 빠르고, CPU는 절반이며, 7스피커 세트 재생·녹음 전체 시간은 PortAudio와 0.04% 이내입니다. 헤드폰 스윕 전체 시간만 0.27%(17 ms) 더 걸리는데, shared 모드 버퍼를 4주기(40 ms)로 둔 대가입니다(3·2주기는 10회 모두 렌더 underrun). 루프백 무결성은 10회 모두 내부 누락 0이었고, 두 번 관측된 480프레임 외부 누락은 독립 파이썬 스트림도 똑같이 놓쳐 Windows 엔진 또는 VB-Cable 쪽임을 확인했습니다(`docs/rust/HARDWARE.md`에 판별법 기록). 캡처 경로는 이벤트 구동 + 패킷 드레인으로 바꾸고 exclusive 거부를 장치별로 기억합니다. 헤드폰 스윕이 아직 PortAudio보다 느리므로 등록부의 `perf.impulcifer-audio-io`·`perf.impulcifer-sys-win`은 planned로 두었습니다(Codex #198). MMCSS 스레드 승격(PA05b)으로도 2·3주기 버퍼는 underrun이 남아 4주기를 유지합니다.
...
#### ⭐ 새로운 기능 / 개선
- **파일 없는 스윕 재생(즉석 생성)이 레코더 기본이 됨**: REW처럼 sweep을 재생 시점에 메모리에서 생성한다(`core/sweep_signal.py`). 기본 파라미터의 생성 시퀀스는 같은 머신에서 파일로 저장해 재생하는 것과 **비트 단위로 동일**하고(인메모리 PCM_32 왕복 양자화), 번들 `sweep-seg-…wav` 대비로는 최대 1 float32 ULP 이내다(플랫폼별 libm 차이 — BRIR 해시를 같은 머신 자기비교로 판정하는 것과 같은 이유). 기존 임펄사이퍼 결과물과 완전 호환 — `tests/test_sweep_signal.py`가 이 계약을 고정. 파일 재생 모드는 특수/비통상 녹음용으로 유지된다. CTk(Stable/Studio 스킨)와 WebView 모두 적용.
...
#### ⚡ 성능 개선
- **플롯 렌더링 병렬화**: figure들이 서로 독립이므로 워커 풀에서 병렬 렌더링한다. 워커 수는 하드코딩 상한 대신 가용 메모리 기반으로 산정한다(`core.parallel_utils.plot_worker_count`: 가용 메모리 절반을 워커당 1.5GiB 예산으로 나눠 CPU 수·작업 수와 min) — 예: 가용 64GiB/24스레드는 figure 14장 전부 병렬(14 워커), 가용 16GiB는 5 워커, 측정 불가 시 보수적 상한 4. 4코어 기준 14 figure 렌더링이 28.5초 → 10.9초로 약 2.6배 단축.

#### ⭐ 개선
- **`generate_ipd_bokeh_layout()`의 `unwrap_phase` 인자 제거**: 크로스 스펙트럼 위상각은 본질적으로 주각(±180°)이라 해당 옵션이 정의되지 않으며, 기존 `False` 경로가 반환하던 ±360° 값도 y축 고정(±180°) 때문에 표시되지 않았다. 조용히 무시하는 대신 명시적으로 제거.

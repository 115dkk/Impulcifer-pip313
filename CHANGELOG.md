...
- **Studio 작업 체크리스트**: Studio의 Activity 카드가 CLI 로그 나열을 넘어 BRIR 파이프라인 10단계(측정 열기 → … → BRIR 쓰기)를 "✓ 완료 / ▸ 진행 중 / ○ 예정" 체크리스트로 표시한다. 로거 메시지를 같은 로케일 테이블의 스테이지 문자열과 prefix 매칭하므로 표시 언어와 무관하게 동작한다.

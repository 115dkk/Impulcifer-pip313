...
#### 🐛 버그 수정
- **모노 sweep WAV에서 `tuple index out of range` 해소**: `data/sweep-6.15s-...wav` 같은 1채널 헤드폰 보정용 sweep을 `core/recorder.py::play_and_record()`가 `read_audio()`를 `expand=False` 기본값으로 호출해 1D `(samples,)` 배열을 받고 `data.shape[1]`에서 `IndexError`를 일으키던 회귀를 수정. `expand=True`로 호출해 항상 2D `(channels, samples)` 형태로 정규화하고, 1채널 입력은 자동으로 스테레오로 브로드캐스트해 양쪽 귀에 동일 sweep이 재생되도록 했다. 회귀 테스트 추가.
- **TrueHD+Atmos MLP 무성 녹음 방지**: 번들된 `data/11cmaster.mlp` / `data/13cmaster.mlp`는 7.1 베드 + Atmos 오브젝트로 인코딩된 마스터 파일이라 ffmpeg가 베드의 8채널만 디코드하고 11/13채널 height 오브젝트는 복원할 수 없다(파일명과 달리 디스크리트 출력 불가). `core.constants.CHANNEL_LAYOUT_MAP`에 없는 레이아웃을 가진 `.mlp/.thd/.truehd` 파일이 들어오면 더 이상 조용히 7.1 베드만 재생하며 녹음하지 않고, 사용자에게 "Atmos object channels cannot be played as discrete outputs … please use a multi-channel sweep WAV generated by Impulcifer instead"라는 명확한 에러를 던진다. 회귀 테스트 추가.

...
Run every command in the foreground. Never use background execution. The caller guarantees that no other worker and no build runs while this packet is measured, so do not query the process list and do not stop or ask about any process; in the environment section write that other processes were not queried and the caller kept the machine quiet. Never terminate any user process. The Python side runs with `py -3.14` and the free-threaded venv named in the template. Start measuring immediately; there is nothing to confirm with the caller before the commands below.

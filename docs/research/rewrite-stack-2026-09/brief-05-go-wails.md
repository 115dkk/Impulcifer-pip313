...
## Your dimension: deep dive on stack S3, Go + Wails

Read E:/Impulcifer/webview_ui/app.js (all of it) to judge the port effort from window.pywebview.api.* and the poll_job model to Wails bindings and events. Then evaluate as of 2026-09:

1. Wails v2 versus v3: is v3 stable and released? Release notes, known blockers, migration state, maintainer activity.
2. Platform webviews (WebView2, WKWebView, WebKitGTK): the same Linux caveats as Tauri? Windows WebView2 runtime handling in the Wails installer.
3. Bindings: v3 generates TypeScript / JavaScript bindings. Do they work with plain vanilla JS and no bundler? Can the project keep zero node_modules at runtime and run only `tsc --checkJs` in CI? What does the dev loop look like without Vite?
4. Events and streaming for progress; passing large arrays to the frontend (float32 plot data); dialogs; window theming (dark title bar).
5. Updater: Wails has no built-in updater. Options: does Velopack have a Go binding (verify); go-selfupdate; minio/selfupdate; a custom Velopack-CLI-driven flow. Code signing.
6. Packaging: NSIS on Windows, dmg on macOS, AppImage on Linux; cross-compiling with cgo (audio libraries need cgo, so per-OS builds or zig cc); binary size.
7. Go numerics for this workload: reference only (another analyst covers DSP in depth), but state the biggest gaps you see (bounded least squares, minimum phase, Kaiser polyphase resampling, cubic splines on a log axis) and whether gonum covers them.
8. Goroutine parallelism for per-speaker work; GC behaviour and memory with large float64 slices; float64 performance relative to Rust and C++ (published benchmarks).
9. PyPI continuity from Go: c-shared plus ctypes or cffi. Assess honestly.
10. Three to five real Wails apps of comparable complexity, with links, and any audio or numeric desktop work in Go with reported pain.

Deliver a proposed architecture: module layout, job model, IPC and events, plots. Rank the risks. Give a verdict.

Report path: {TMP}/report-05-go-wails.md

...
## Your dimension: audio I/O feasibility per language (requirement R1)

First read E:/Impulcifer/core/recorder.py fully and E:/Impulcifer/application/impulcifer_service.py lines 300-350 (device listing) so you know the exact behaviour to reproduce: host-API grouping, device matching by name plus host-API suffix, the DirectSound/MME/WASAPI preference order, min_channels checks, playback with sd.play(blocking=True) in the main thread while sd.rec runs in a worker thread, and the up-to-16-channel output layouts (E:/Impulcifer/core/constants.py SWEEP_TRACK_LAYOUTS: mono, stereo, 5.1, 7.1, 7.1.4, 7.1.6).

Evaluate for each of Rust, C++, Go, C#/.NET, and more briefly Dart/Flutter and Kotlin/JVM:
1. Which libraries or bindings give device enumeration WITH host-API grouping and multi-channel float32 output (8 or more, ideally 16 channels) plus 2-channel input. Check at least: Rust (cpal, rust-portaudio / portaudio-rs, miniaudio-rs, direct wasapi / coreaudio-rs / alsa crates), C++ (PortAudio, RtAudio, miniaudio, JUCE AudioDeviceManager, SDL3 audio), Go (gordonklaus/portaudio, malgo = miniaudio, ebitengine/oto), .NET (PortAudioSharp2, NAudio and its non-Windows status, MiniAudioEx, SDL3-CS), Flutter (FFI to the Rust/C++ libs; any pure-Dart option), JVM (javax.sound.sampled limits; PortAudio JNI bindings).
2. For each: maintenance status in 2026 (last release date, open issue count, notable open bugs), cross-platform coverage, cgo / FFI build burden, license.
3. ASIO: what it takes per library (Steinberg ASIO SDK licensing and redistribution rules; PortAudio built with ASIO; cpal's asio feature; miniaudio's ASIO status; JUCE ASIO). Is ASIO needed at all if WASAPI shared or exclusive gives multichannel output? Give a recommendation for this project.
4. WASAPI multichannel above 8 channels on Windows: how 7.1.4 / 7.1.6 layouts behave through consumer drivers versus pro interfaces, and whether each library exposes channel counts and channel masks properly.
5. Two independent streams (separate play and record) versus one duplex stream: which libraries support which, and how the current Python design maps onto each. Note any library where a blocking "play this buffer to completion" call must be hand-built from a callback.
6. Verdict table: language -> best library -> risk level (low / medium / high) -> one-line reason. Then a second table for the "second-best" fallback per language.

Report path: {TMP}/report-01-audio-io.md

...
class ImpulseResponseEstimator(object):
    ...
    def sweep_sequence(self, speakers, tracks):
        ...
        # Remap channels
        if tracks == '7.1.6':
            # 14-channel Dolby Atmos 7.1.6 layout. Used for the bundled
            # split sweep set (FL,FR / FC / SL,SR / BL,BR) when the user
            # has a 14-channel output device and wants per-group capture.
            # The seven ground speakers occupy their standard 7.1
            # positions; height channels follow the same TBL/TBR/TSL/TSR
            # ordering as ``TRUEHD_13CH_ORDER``.
            standard_order = 'FL FR FC LFE BL BR SL SR TFL TFR TBL TBR TSL TSR'.split()
            n_tracks = 14
        elif tracks == '7.1':
...
def create_cli():
    ...
    arg_parser.add_argument('--tracks', type=str, required=False, default='mono',
                            help='WAV file track configuration. Supported values are "mono", "stereo", "5.1", '
                                 '"7.1" and "7.1.6". This should be set according to sound card. Supported speaker '
                                 'names for "stereo" are "FL" and "FR". Supported speaker names for "7.1" are "FL", '
                                 '"FR", "FC", "BL", "BR", "SL" and "SR". Supported speaker names for "5.1" are "FL", '
                                 '"FR", "FC", "SL", "SR", "BL" and "BR". "7.1.6" extends "7.1" with the four height '
                                 'speakers TFL/TFR/TBL/TBR plus TSL/TSR (14-channel Atmos). "mono" will force speakers '
                                 'to "FL".')

...
# the WAVE channel-mask order; the height layouts append the top layer in
# TRUEHD_13CH_ORDER's TFL TFR [TSL TSR] TBL TBR order. "stereo" and "mono"
# are positional and handled separately in ``sweep_sequence``.
SEQUENCE_TRACK_ORDERS = {
    '5.1': 'FL FR FC LFE BL BR'.split(),
    '7.1': 'FL FR FC LFE BL BR SL SR'.split(),
    '7.1.4': 'FL FR FC LFE BL BR SL SR TFL TFR TBL TBR'.split(),
    '7.1.6': 'FL FR FC LFE BL BR SL SR TFL TFR TSL TSR TBL TBR'.split(),
}
...
class ImpulseResponseEstimator(object):
    ...
    def sweep_sequence(self, speakers, tracks):
        ...
        the FL is silent. With "stereo" any one or two speakers may be given; they map positionally to the two output
        channels (speakers[0] -> left, speakers[1] -> right) so a stereo-only user can reposition a speaker pair to
        capture groups such as [SL, SR], [BL, BR] or [FC].
        ...
        Args:
            ...
            tracks: Tracks configuration. "7.1.6", "7.1.4", "7.1", "5.1", "stereo" or "mono".
                    The height layouts extend the 7.1 WAVE order with top-layer speakers
                    (TFL TFR [TSL TSR] TBL TBR) so surround sequences can excite height
                    channels too.
        ...
        if tracks in SEQUENCE_TRACK_ORDERS:
            ...
            for speaker in speakers:
                if speaker not in SPEAKER_NAMES:
                    raise ValueError('Speaker name "{speaker}" is not a recognised speaker.'.format(
                        speaker=speaker
                    ))
            standard_order = list(speakers)
            n_tracks = 2
        elif tracks == 'mono':
            standard_order = ['FL']
            speakers = ['FL']
...
def create_cli():
    ...
    arg_parser.add_argument('--tracks', type=str, required=False, default='mono',
                            help='WAV file track configuration. Supported values are "mono", "stereo", "5.1", '
                                 '"7.1", "7.1.4" and "7.1.6". This should be set according to sound card. "stereo" '
                                 'accepts any one or two speakers, mapped positionally to the left/right channels. '
                                 'Supported speaker names for "7.1" are "FL", "FR", "FC", "BL", "BR", "SL" and "SR"; '
                                 '"7.1.4" adds "TFL", "TFR", "TBL", "TBR" and "7.1.6" additionally "TSL", "TSR". '
                                 'Supported speaker names for "5.1" are "FL", "FR", "FC", "SL", "SR", "BL" and "BR". '

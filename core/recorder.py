...
def play_and_record(
    ...
    elif os.path.splitext(play)[1].lower() in {'.mlp', '.thd', '.truehd'}:
        # TrueHD/MLP files whose layout isn't in CHANNEL_LAYOUT_MAP
        # almost always carry Atmos object content over a 7.1 bed —
        # FFmpeg only decodes the bed and silently drops the height/wide
        # objects, so the user ends up with fewer channels than the file
        # name promises. Surface that explicitly.
        raise ValueError(
            f"TrueHD/MLP file '{os.path.basename(play)}' decodes to "
            f"{data.shape[0]} channels with no recognized layout. Atmos "
            "object channels cannot be played as discrete outputs through "
            "a normal audio interface — please use a multi-channel sweep "
            "WAV generated by Impulcifer instead."
        )

    # Broadcast a 1-channel sweep (the bundled headphone-comp mono file)
    # to stereo so both ears receive the signal. Multi-channel sweeps
    # already carry per-speaker routing and are left untouched.
    if data.shape[0] == 1:
